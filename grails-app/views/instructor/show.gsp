	...
	<body>
		...
		<div id="show-instructor" class="content scaffold-show" role="main">
			...
			<ol class="property-list instructor">
				...
				<g:if test="${instructorInstance?.classTaught}">
				<li class="fieldcontain">
					<span id="classTaught-label" class="property-label"><g:message code="instructor.classTaught.label" default="Class Taught" /></span>
					
						<g:each in="${instructorInstance.classTaught}" var="c">
						<span class="property-value" aria-labelledby="classTaught-label"><g:link controller="regClass" action="show" id="${c.id}">${c?.encodeAsHTML()}</g:link></span>
						</g:each>
